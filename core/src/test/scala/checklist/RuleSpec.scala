...
class ConverterRulesSpec extends FreeSpec with Matchers {
  ...
  "mapValue" - {
    "string keys" in {
      val rule = mapValue[Id, String, String]("foo")
    ...
    "int keys" in {
      val rule = mapValue[Id, Int, String](123)
...
class CombinatorRulesSpec extends FreeSpec with Matchers {
  ...
  "contramapPath" in {
    val rule: Rule[Id, Int, Int] = Rule.gt[Int](0, errors("fail")).contramapPath("n")(_ + 1)
    rule(+1) should be(Ior.right(2))
    rule(0) should be(Ior.right(1))
...
class CatsRuleSpec extends FreeSpec with Matchers {
  ...
  def getField(name: String): Rule[Id, Map[String, String], String] =
    pure[Id, Map[String, String], String](_.get(name).map(Ior.right).getOrElse(Ior.left(errors(s"Field not found: ${name}"))))
